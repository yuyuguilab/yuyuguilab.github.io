...
```
├── _config.yml           # Hexo 站点主配置（英文站，根路径 /）
├── _config.zh.yml        # 中文站覆盖：root=/zh/ + source
...
├── tools/build.js        # 双语构建脚本（生成英文根站 + 中文 /zh/ + 语言切换注入）
...
| 路径 | 内容 |
|------|------|
| `/` | 英文站（默认主站，根路径直接是完整首页，SEO 友好） |
| `/zh/` | 中文站 |

根路径为完整英文首页（无过渡跳转页）；中文浏览器访问时通过 hreflang + 极小内联脚本静默引导至 `/zh/`，不阻塞渲染。每页顶部导航有语言切换按钮，在 `/...` ↔ `/zh/...` 同路径间切换。
...
**注意**：英文文章的 `categories` 也写中文分类名（如 `[商业]`），这样中英文 URL 完全对称（`/business/x/` ↔ `/zh/business/x/`）。
